...
function requireRole(...allowedRoles) {
    return function(req, res, next) {
        ...
        if (normalizedAllowedRoles.includes(userRole)) {
            console.log(`✅ Authorized: ${req.currentUser.email} has role ${req.currentUser.role}`);
            return next();
        }
        
        // User does not have required role
        console.log(`❌ Access denied: ${req.currentUser.email} (${req.currentUser.role}) tried to access ${req.path}`);
        ...
        return res.status(403).send(generateAccessDeniedHTML(req.currentUser.role, allowedRoles));
    };
}
